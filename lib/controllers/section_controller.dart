...
class SectionController {
  ...
  Future<Response> Function(Request request, String id)
      get getSectionByIdHandler => _getSectionById;

  // Public handlers for routes that require authentication via middleware
  Handler get createSectionHandler => _createSection;
  Future<Response> Function(Request request, String id)
      get updateSectionHandler => _updateSection;
  Future<Response> Function(Request request, String id)
      get deleteSectionHandler => _deleteSection;
  ...
  Future<Response> _getAllSections(Request request) async {
    ...
    try {
      final results = await _db.query(
          'SELECT id, title, content, "imageUrl" FROM sections ORDER BY id');

      // Use Section.fromJson which should be generated by json_serializable
      final sections =
          results.map((row) => Section.fromJson(row.toColumnMap())).toList();
  ...
  Future<Response> _createSection(Request request) async {
    ...
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(
          403, 'Forbidden: Only admin users can create sections.');
    }
  ...
  Future<Response> _updateSection(Request request, String id) async {
    ...
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(
          403, 'Forbidden: Only admin users can update sections.');
    }
  ...
  Future<Response> _deleteSection(Request request, String id) async {
    ...
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(
          403, 'Forbidden: Only admin users can delete sections.');
    }
  ...
  String? _validateSectionPayload(Map<String, dynamic> body) {
    ...
    if (body['imageUrl'] == null ||
        body['imageUrl'].toString().trim().isEmpty) {
