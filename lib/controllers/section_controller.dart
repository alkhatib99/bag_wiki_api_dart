...
class SectionController {
  ...
  // Public handlers for routes that don't require authentication via middleware
  Handler get getAllSectionsHandler => _getAllSections;
  Future<Response> Function(Request request, String id) get getSectionByIdHandler => _getSectionById;

  // Public handlers for routes that require authentication via middleware
  Handler get createSectionHandler => _createSection;
  Future<Response> Function(Request request, String id) get updateSectionHandler => _updateSection;
  Future<Response> Function(Request request, String id) get deleteSectionHandler => _deleteSection;

  // --- Private Route Handlers ---

  /// Handles GET / requests to fetch all sections.
  Future<Response> _getAllSections(Request request) async {
    // No authentication check here, as this is for public access
    try {
      final results = await _db.query('SELECT id, title, content, "imageUrl" FROM sections ORDER BY id');
  ...
  Future<Response> _getSectionById(Request request, String id) async {
    ...
    try {
      final results = await _db.query(
        'SELECT id, title, content, "imageUrl" FROM sections WHERE id = @id',
  ...
  Future<Response> _createSection(Request request) async {
    ...
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(403, 'Forbidden: Only admin users can create sections.');
    }
    ...
    try {
      ...
      final result = await _db.query(
        ...
        RETURNING id, title, content, "imageUrl"
        ''',
  ...
  Future<Response> _updateSection(Request request, String id) async {
    ...
    final user = request.context['user'] as Map<String, dynamic>?;
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(403, 'Forbidden: Only admin users can update sections.');
    }
  ...
  Future<Response> _deleteSection(Request request, String id) async {
    ...
    final user = request.context['user'] as Map<String, dynamic>?;
    if (user == null || user['role'] != 'admin') {
      return _jsonErrorResponse(403, 'Forbidden: Only admin users can delete sections.');
    }
